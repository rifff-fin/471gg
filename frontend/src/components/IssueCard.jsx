  ...
  <article className="issue-card">
    <div className="issue-card__content">
      <div className="issue-card__meta">
        <span className={`status status--${statusClass(complaint.status)}`}>{complaint.status || "Pending"}</span>
        <span>{complaint.category || "General"}</span>
        {complaint.ward && <span><FaLocationDot /> {complaint.ward}</span>}
      </div>
      <Link className="issue-card__title" to={`/complaints/${complaint._id || ""}`}>{title}</Link>
      <p>{complaint.description || "No description was provided."}</p>
      <div className="issue-card__footer">
        <span><FaClock /> Reported {formatDate(complaint.createdAt)}</span>
        <span><FaComment /> {commentCount} updates</span>
        <span>Assigned to {complaint.department || "triage"}</span>
      </div>
    </div>
    <div className="vote-box">
      <button type="button" onClick={() => onVote?.(complaint)} disabled={voting || !complaint._id} aria-label={`Support ${title}`}>
        <FaArrowUp />
      </button>
