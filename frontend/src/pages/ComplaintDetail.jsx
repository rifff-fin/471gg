...
const ComplaintDetail = () => {
  ...
  return <main className="site-shell detail-page"><Link className="back-link" to="/"><FaArrowLeft /> All community reports</Link>{notice && <div className="notice">{notice}</div>}<div className="detail-layout"><article className="detail-story"><div className="issue-card__meta"><span className="status">{complaint.status}</span><span>{complaint.category}</span><span>{complaint.department}</span></div><h1>{complaint.title}</h1><p className="detail-description">{complaint.description}</p>{complaint.images?.length > 0 && <div className="image-grid">{complaint.images.map((image) => <img key={image.url} src={image.url} alt="Complaint evidence" />)}</div>}<div className="location-line"><FaLocationDot /> {complaint.ward || "Ward to be confirmed"} <span>·</span> Reported {formatDate(complaint.createdAt)}</div><ComplaintLocationMap location={complaint.location} /><div className="timeline"><div className="timeline-heading"><h2>Public activity</h2><span>{activities.length} events</span></div>{visibleActivities.map((entry, index) => <div className="timeline-item" key={`${entry.createdAt}-${index}`}><span></span><div><strong>{entry.action.replaceAll("_", " ")}</strong><p>{entry.message}</p>{entry.createdAt && <small>{formatDate(entry.createdAt)}</small>}</div></div>)}{activities.length > 5 && <button type="button" className="text-button timeline-toggle" onClick={() => setShowAllActivity((current) => !current)}>{showAllActivity ? "Show fewer activities" : `Show ${activities.length - 5} more activities`}</button>}</div></article><aside className="detail-sidebar"><div className="support-card"><FaArrowUp /><strong>{complaint.upvotes || 0}</strong><span>people support this issue</span><button className="button button--primary" type="button" onClick={vote} disabled={saving}>Support this issue</button><small>One verified support per account.</small></div><div className="facts-card"><h2>Issue details</h2><dl><div><dt>Department</dt><dd>{complaint.department}</dd></div><div><dt>Priority</dt><dd>{complaint.priorityLevel}</dd></div><div><dt>Current state</dt><dd>{complaint.status}</dd></div></dl></div></aside></div><section className="discussion"><div className="section-heading"><div><p className="eyebrow">Conversation</p><h2><FaComment /> Public updates</h2></div><span>{complaint.comments?.length || 0} posts</span></div>{user ? <form className="comment-form" onSubmit={postComment}><textarea value={comment} onChange={(event) => setComment(event.target.value)} placeholder="Share a useful update with your community…" disabled={saving} rows="4" /><button className="button button--primary" disabled={saving || !comment.trim()}><FaPaperPlane /> Post update</button></form> : <div className="sign-in-prompt">Want to add a public update? <Link to="/login">Sign in to join the conversation.</Link></div>}<div className="comment-list">{complaint.comments?.length ? complaint.comments.slice().reverse().map((item, index) => <CommentThread key={`${item.createdAt}-${index}`} item={item} complaintId={id} canReply={Boolean(user)} onRefresh={load} />) : <div className="empty-card">No public updates yet.</div>}</div></section>{privateChatAllowed && <PrivateChat complaintId={id} currentUserId={user._id || user.id} />}</main>;
};
